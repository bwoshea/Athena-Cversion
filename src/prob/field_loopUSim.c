...
void problem(DomainS *pDomain)
{
  ...
  Real rad,amp,vflow,drat,diag,radin,radout,startang,stopang;
  Real ***az,***ay,***ax;
#ifdef MHD
  int ku;
#endif
...
/* Read initial conditions, diffusion coefficients (if needed) */
  ...
  radin = par_getd("problem","radin");
  radout = par_getd("problem","radout");
  startang = par_getd("problem","startang");
  stopang= par_getd("problem","stopang");
 ...
 */
  ...
  for (i=is; i<=ie; i++) {
     ...
     cc_pos(pGrid,i,j,k,&x1c,&x2c,&x3c);
     if ((x1c*x1c + x2c*x2c + x3c*x3c) > radin*radin && (x1c*x1c + x2c*x2c + x3c*x3c) < radout*radout && (atan(x1c/x2c)<(stopang*PI-PI)) && (atan(x1c/x2c)>(startang*PI-PI)) && (x2c > 0.) ) {
